    ...
    if native_name.endswith(".pdb"):
        ...
        if os.path.isdir(decoy_dir):
            for decoy_name in os.listdir(decoy_dir):
                pc_file = arg.root_dir + 'pointclouds/' + \
                    subfolder + decoy_name[:-4] + '.pickle'
                if os.path.isfile(pc_file):
                    ...
                    try:
                        ...
                        # Create "point cloud"
                        pc = np.array(atFeat.sqldb.get(
                            'x,y,z,eps,sig,charge,chainID', rowID=indA+indB))

                        # Get iRMSD
                        sim = StructureSimilarity(
                            decoy_dir+'/'+decoy_name, arg.root_dir+'natives/'+native_name)
                        irmsd = sim.compute_irmsd_fast(
                            method='svd', izone=native_name[:4]+'.izone')
                        ...
                        # Convert to float because, unfortunately, the sql returns everything as strings
